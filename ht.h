...
public:
    ...
    HASH_INDEX_T next() 
    {
      ...
      unsigned int loc;
      loc = (this->start_ + this->numProbes_ * dhstep_) % this-> m_;
...
{
  if((bucketFilled_+1)/static_cast<double>(CAPACITIES[mIndex_]) >= resizeAlpha_)
  {
    resize();
  }
  unsigned int loc = probe(p.first);
  if(loc == npos)
  {
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
  if(mIndex_ == 27)
  {
    //took me a while to count it, not sure if this guard is needed in our case though
    throw std::logic_error("Out of Capacities");
  }
  ...
  {
    HashItem* oldItem = temp[i];
    if( oldItem != nullptr && !oldItem->deleted)
    {
      insert(oldItem->item);
    }
    delete oldItem;
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    while(Prober::npos != loc)
    {
        ...
        else if(kequal_(table_[loc]->item.first, key) && !table_[loc]->deleted && table_[loc] != nullptr) {
            return loc;
        }
        loc = prober_.next();
        totalProbes_++;
